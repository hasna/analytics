...
server.registerTool(
  ...
  async ({ name, period }) => {
    ...
    if (!kpi) {
      return {
        content: [{ type: "text", text: `KPI '${name}' not found.` }],
        isError: true,
      };
    }
    return { content: [{ type: "text", text: JSON.stringify(kpi, null, 2) }] };
  ...
  async ({ name, days }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ trend, count: trend.length }, null, 2),
        },
      ],
    };
  }
  ...
  async (params) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ kpis, count: kpis.length }, null, 2),
        },
      ],
    };
  }
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    if (!dashboard) {
      return {
        content: [
          { type: "text", text: `Dashboard '${id}' not found.` },
        ],
        isError: true,
      };
    }
...
server.registerTool(
  "get_business_health",
  {
    ...
    description:
      "Get overall business health summary across all KPI categories.",
    inputSchema: {},
  },
  async () => {
...
server.registerTool(
  "generate_executive_summary",
  {
    ...
    description:
      "Generate an AI-powered executive summary of KPI trends.",
    inputSchema: {},
  },
  async () => {
