...
export function getKpiById(id: string): Kpi | null {
  const db = getDatabase();
  const row = db
    .prepare("SELECT * FROM kpis WHERE id = ?")
    .get(id) as KpiRow | null;
...
export function getKpiTrend(name: string, days = 30): Kpi[] {
  const db = getDatabase();
...
export function generateReport(input: GenerateReportInput): Report {
  ...
  for (const [category, kpis] of categories) {
    ...
    for (const kpi of kpis) {
      lines.push(
        `  ${kpi.name}: ${kpi.value}${kpi.period ? ` (${kpi.period})` : ""}`
      );
    }
    lines.push("");
  }
...
export function getBusinessHealth(): BusinessHealth {
  ...
  const totalKpis = (
    db
      .prepare("SELECT COUNT(DISTINCT name) as count FROM kpis")
      .get() as { count: number }
  ).count;
  ...
  const reportCount = (
    db.prepare("SELECT COUNT(*) as count FROM reports").get() as {
      count: number;
    }
  ).count;

  const dashboardCount = (
    db.prepare("SELECT COUNT(*) as count FROM dashboards").get() as {
      count: number;
    }
  ).count;
...
Provide a brief, actionable executive summary in 3-5 sentences.`;
  ...
  if (latestKpis.length === 0) {
    lines.push(
      "No KPIs have been recorded yet. Start tracking key metrics to enable business health reporting."
    );
    return lines.join("\n");
  }

  lines.push(
    `Tracking ${health.total_kpis} unique KPI(s) across ${health.categories.length} category(s).`
  );
