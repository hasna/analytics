...
const tempDir = mkdtempSync(join(tmpdir(), "open-analytics-test-"));
process.env["ANALYTICS_DIR"] = tempDir;
...
describe("KPIs", () => {
  ...
  test("get KPI trend", () => {
    ...
    expect(trend[0]!.recorded_at <= trend[trend.length - 1]!.recorded_at).toBe(
      true
    );
  });
