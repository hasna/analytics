...
program
  .name("analytics")
  .description("Business analytics — KPIs, dashboards, reports")
  .version("0.0.1");

// --- KPIs ---

const kpiCmd = program.command("kpi").description("KPI management");
...
kpiCmd
  ...
  .action((opts) => {
    const kpi = recordKpi({
      ...
      value: parseFloat(opts.value as string),
      category: opts.category as string | undefined,
      source_service: opts.source as string | undefined,
...
kpiCmd
  ...
  .action((name: string, opts) => {
    const kpi = getKpi(name, opts.period as string | undefined);
    if (!kpi) {
...
kpiCmd
  ...
  .option("--json", "Output as JSON", false)
  .action((name: string, opts) => {
    const trend = getKpiTrend(name, parseInt(opts.days as string));
    ...
    if (opts.json) {
      ...
      if (trend.length === 0) {
        console.log(
          `No trend data for '${name}' in the last ${opts.days as string} days.`
        );
        return;
      }
      console.log(`Trend for ${name} (last ${opts.days as string} days):`);
...
const dashCmd = program.command("dashboard").description("Dashboard management");
...
const reportCmd = program.command("report").description("Report management");
  ...
  .action((opts) => {
    ...
    if (opts.json) {
      ...
      for (const r of reports) {
        ...
        console.log(
          `  [${r.type}] ${r.name}${period} — ${r.generated_at}`
        );
      }
    ...
    if (opts.json) {
      ...
      if (health.categories.length > 0) {
        ...
        for (const cat of health.categories) {
          console.log(
            `    ${cat.category}: ${cat.count} KPI(s), latest value: ${cat.latest_value}`
          );
        }
      }
